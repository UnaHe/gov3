...
{% block content %}
    ...
    <div class="wrap">
        <div class="title_g">
            <a class="return center" href="{{ url('status/index?pid=' ~ title_info.project_id ~ "&did=" ~ title_info.department_id) }}">
                <img src="{{ url('home/style/img/return_03.png') }}" />
            </a>
            <h5 class="tetle_font">{{ title_info.department_name }}</h5>
            <a class="Reserved"></a>
            <div class="lf_BreadCrumbs">
                <span class="lf_BreadCrumbs_1" onclick="location.href='{{ url('notice?pid=' ~ title_info.project_id ~ "&did=" ~ title_info.department_id) }}'">
                    <img src="{{ url('home/style/img/jkl.png') }}">
                </span>
                <span class="lf_BreadCrumbs_2" onclick="location.href='{{ url('department/detail?pid=' ~ title_info.project_id ~ "&did=" ~ title_info.department_id) }}'">
            ...
            $('.wrap').dropload({
                ...
                loadUpFn: function (me) {
                    ...
                    $.ajax({
                        type: 'POST',
                        dataType: 'JSON',
                        url: "{{ url('status/workerStatusList?pid=' ~ title_info.project_id ~ '&did=' ~ title_info.department_id ~ '&page=1') }}",
                        data: {
                            "{{ _csrfKey }}": "{{ _csrf }}",
                        },
                ...
                loadDownFn : function(me) {
                    ...
                    if ((len % page_size == 0) && have_data && flag) {
                        ...
                        $.ajax({
                            type: 'POST',
                            dataType: 'JSON',
                            url: "{{ url('status/workerStatusList?pid=' ~ title_info.project_id ~ '&did=' ~ title_info.department_id ~ '&page=') }}" + page,
                            data: {
                                "{{ _csrfKey }}": "{{ _csrf }}",
                            },
                            success: function (data) {
