        ...
        <div class="result_wrap">
            <form action="{{url('admin/users')}}" method="post" id="add-form" name="add-form">
                ...
                <table class="add_tab">
                    ...
                    {% endif %}
                    {% Include 'layout/common_tr' with ['type': 2, 'department_id':old('department_id') ,'section_id':old('section_id')] %}
                    
                    <tr>
                        <th><i class="require">*</i> 姓名：</th>
                        <td>
                            <input type="text" class="lg" name="user_name" value="{{old('user_name')}}" required/>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <th>寸照：</th>
                        <td>
                            <input type="hidden" name="user_image" class="user_image" value="{{old('user_image')}}">
                            <input id="thumbnail" name="thumbnail" type="file" multiple onchange="UpLoadFile()">
                            <img src="@if(old('user_image')){{Config("constants.upload_url")}}{{old('user_image')}}@endif" width="100" id="previewImage" style="max-width: 75px; max-height:100px;{{old('user_image') ? '': 'display: none;'}}"/>
                            <input type="button" value="移除图片" class="btn_remove back btn btn-info"
                                   style="{{old('user_image') ? '': 'display: none;'}}">
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i> 性别：</th>
                        <td>
                            <input type="radio" name="user_sex" id="user_sex_1" value="1" {{old('user_sex')=='0' ? '' :'checked'}}  required/><label for="user_sex_1">男</label>
                            <input type="radio" name="user_sex" id="user_sex_0" value="0" {{old('user_sex')=='0' ? 'checked'  :''}} required/><label for="user_sex_0">女</label>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i> 年龄：</th>
                        <td>
                            <input type="number" class="lg" name="user_age" required min="1" max="100" value="{{old('user_age')}}"/>
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i> 手机号码：</th>
                        <td>
                            <input type="text" class="lg" name="user_phone" value="{{old('user_phone')}}" required />
                        </td>
                    </tr>
                    <tr>
                        <th><i class="require">*</i> 状态：</th>
                        <td>
                            <input type="radio" name="user_status" id="user_status_1" value="1" {{old('user_status')=='0' ? '' :'checked'}} required/><label for="user_status_1">正常在职</label>
                            <input type="radio" name="user_status" id="user_status_0" value="0" {{old('user_status')=='0' ? 'checked' :''}} required/><label for="user_status_0">不在职</label>
                        </td>
                    </tr>
                    <tr>
                        <th>职务：</th>
                        <td>
                            <input type="text" class="lg" name="user_job" value="{{old('user_job')}}">
                        </td>
                    </tr>
                    <tr>
                        <th>职务内容：</th>
                        <td>
                            <textarea name="user_intro">{{old('user_intro')}}</textarea>
                        </td>
                    </tr>
                    <tr>
                        <th>他的留言：</th>
                        <td>
                            <textarea name="user_comments">{{old('user_comments')}}</textarea>
                        </td>
                    </tr>
                    <tr>
                        <th></th>
                ...
                getImageWidthAndHeight(files[0], function (obj) {
                    ...
                    if (obj.width > 295 || obj.height > 415 || proportion < 0.65 || proportion > 0.75) {
                        ...
                        xhr.onreadystatechange = function () {
                            if (xhr.readyState == 4) {
                                if (xhr.status == 200) {
                                    $('#previewImage').attr('src', '{{Config("constants.upload_url")}}' + xhr.responseText);
